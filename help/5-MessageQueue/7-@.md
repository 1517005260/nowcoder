					...
					<div class="media-body">
						...
						<ul class="list-unstyled mt-4 bg-gray p-3 font-size-12 text-muted">
							...
							<li class="pb-3 pt-3">
								<form method="post" th:action="@{|/comment/add/${post.id}|}">
									<div>
										<input type="text" class="input-size"  name="content" placeholder="请输入你的观点" required>
										<input type="hidden" name="entityType" value="2"> <!--评论-->
										<input type="hidden" name="entityId" th:value="${cvo.comment.id}">
									</div>
									<div class="text-right mt-2">
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
		...
		// 正则表达式匹配 @用户名
		var mentionPattern = /@([a-zA-Z0-9_]+)/g;
				...
				var promises = matches.map(function(match) {
					...
					return new Promise(function(resolve, reject) {
						getUserIdByUsername(username, function(userId) {
							if (userId !== null) {
								var userLink = '<a href="/community/user/profile/' + userId + '">' + match + '</a>';
